...
export default function AdminPortfolioForm({ onSuccess }: { onSuccess?: () => void }) {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const res = await callApi("/api/portfolio-projects", "POST", payload);
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4 p-4 border rounded bg-white">
      ...
      <Label>Year</Label>
      <Input value={form.year} onChange={e => handleChange("year", e.target.value)} required />
      <Label>Client</Label>
      ...
      <Label>Status</Label>
      <select value={form.status} onChange={e => handleChange("status", e.target.value)} className="border rounded p-2">
        <option value="completed">Completed</option>
        <option value="ongoing">Ongoing</option>
      </select>
      <Button type="submit" disabled={loading}>Submit Project</Button>
    </form>
  );
}
